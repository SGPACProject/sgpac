...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['materia1_submit'])) {
    ...
    if ($punto1 <= 0 || $punto1 > 7) {
        echo "<script>alert('Los puntos de Materia 1 deben estar entre 1 y 12.'); window.location='instrcutor.php';</script>";
        exit();
    }
    ...
    if (!empty($materia1)) {
        ...
        if ($conn->query($query) === TRUE) {
            echo "<script>alert('Materia 1 registrada correctamente.'); window.location='instructor.php';</script>";
        } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['materia2_submit'])) {
    ...
    $puntos_restantes = max(0, 12 - $punto1);
    ...
    if (!empty($materia2)) {
        ...
        if ($conn->query($query) === TRUE) {
            echo "<script>alert('Materia 2 registrada correctamente.'); window.location='instructor.php';</script>";
        } else {
...
<head>
    ...
    <title>Instructor - Gestión de puntos</title>
    <style>
        body {
...
<div style="margin-top: 20px; padding: 15px; background-color: rgba(0, 0, 0, 0.1); border-radius: 5px;">
    ...
    <p style="text-align: justify;">
        ...
        participación en las actividades, corresponde un total de <strong>12 puntos</strong> y estos serán repartidos 
    ...
    <form method="post">
        ...
        <label>Puntos:</label>
        <input type="number" name="punto1" min="1" max="12" value="<?php echo $punto1; ?>" 
               <?php echo $materia1_bloqueada ? 'readonly' : ''; ?> required>
        <br>
    ...
    <form method="post">
        ...
        <label>Puntos:</label>
        <input type="number" name="punto2" min="0" max="<?php echo max(0, 12 - $punto1); ?>" 
